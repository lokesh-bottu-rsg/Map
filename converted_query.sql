...
FROM PR_PLAN PP WHERE PLAN_ID= 398
AND MEDIA_OUTLET_ID= 96 LIMIT 1
...
(CASE WHEN 1 = 1 THEN SUM(PPS.PLANNED_UNITS) ELSE SUM(PPS.PLANNED_GRP) END) UNIT_COUNT
FROM PR_PLAN_SCHEDULE_EPISODE PPS 
JOIN CTE_WEEKS 
JOIN PR_PLAN_PROJECT_EPISODE PPP ON PPS.PLAN_ID=398 
...
SELECT PPS.CAMPAIGN_GOAL_ID,PPS.PROJECT_ID,PPS.EPISODE_ID,WSTART_DATE,WEEK,
(CASE WHEN 1 = 1 THEN SUM(PLANNED_UNITS) ELSE SUM(PPS.PLANNED_GRP) END) UNIT_COUNT
    FROM PR_PLAN_SCHEDULE_EPISODE PPS
    JOIN CTE_WEEKS
